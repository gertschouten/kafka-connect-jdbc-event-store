...
public interface DatabaseDialect extends ConnectionProvider {
   ...
   * {@link #buildInsertStatement(TableId, Collection, TableDefinition)}, since
   * many dialects implement this variant of the method. However, overriding
   * {@link #buildInsertStatement(TableId, Collection, TableDefinition)} is suggested.
   *
   ...
   * @deprecated use {@link #buildInsertStatement(TableId, Collection, TableDefinition)}
   */
  @Deprecated
   ...
   * {@link #buildInsertStatement(TableId, Collection)} to maintain backward
   * compatibility with older versions. Subclasses that override this method do not need to
   * override {@link #buildInsertStatement(TableId, Collection)}.
   *
   * @param table         the identifier of the table; may not be null
   * @param nonKeyColumns the identifiers of the other columns in the table; may not be null but may
   *                      be empty
  ...
  default String buildInsertStatement(
    ...
    return buildInsertStatement(table, nonKeyColumns);
  }
  ...
  default StatementBinder statementBinder(
    ...
    return statementBinder(statement, schemaPair, fieldsMetadata, insertMode, coordinates);
  }
